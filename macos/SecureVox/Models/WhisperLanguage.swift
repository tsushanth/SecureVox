...
extension WhisperLanguage {
    ...
    /// Common languages (shown at top of picker for quick access)
    ...
    /// Languages sorted alphabetically by localized name
    static var sortedLanguages: [WhisperLanguage] {
        var sorted = allLanguages.filter { !$0.isAutoDetect }
            .sorted { $0.localizedName.localizedCompare($1.localizedName) == .orderedAscending }
        sorted.insert(autoDetect, at: 0)
        return sorted
    }

    /// Get display name (localized if available, otherwise fallback to English name)
...
extension WhisperLanguage {
    ...
    /// Falls back to the device locale language (if supported), then autoDetect.
    static func loadSaved() -> WhisperLanguage {
        guard let code = UserDefaults.standard.string(forKey: defaultsKey),
              let language = find(byCode: code) else {
